...
let reset_time_badge = 5000;
let lost = false;
let badge_game_check = false;
let setIntervalID;
let setTimeoutID;
let setTimeoutID1;
let cellLightningTimeout;
  ...
  ws.on("message", (messageReceived) => {
    ...
    if (messageReceived.search("phase_finished") !== -1) {
      ...
      finish = setTimeout(() => {
        ...
        cellLightningTimeout = setTimeout(() => {
          console.log("\n[ARDUINO CALL] Turning OFF cell lightning.");
          arduino.turn_off_cell_lightning();
        }, 10000);
      }, 100);
    }
    ...
    if (messageReceived.search("start") !== -1 && badge_check && badge_game_check) {
      selected_game = JSON.parse(JSON.parse(messageReceived)).data;
      arduino.emitter.emit("selected_game", selected_game);
      em.emit("gameStart");
      //interpret = true;
    }
